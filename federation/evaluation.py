...
# Datensammlung: {algorithm: {variant: {step: {metric: value}}}}
all_data = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))

# Neue Regex: Trenner ist "-"
folder_pattern = re.compile(r"(?P<algo>.+)-(?P<lr>[\d.]+)-(?P<dropout>[\d.]+)-(?P<step>\d+)")
